...
class AddFindDevices(threading.Thread):
    ...
    def run(self):
        if self.method[0] == 'add':
            self.ip_connect(self.method[1])
        else:
            self.network_search()
...
def main():
    if __name__ == "__main__":
        add_find_device_thread = AddFindDevices('add', '192.168.0.100')  # add value for the argument method 'add or find'
